...
{
    this->print();
    FunctionPtr callF = context.theModule->getFunction(this->FunctionName->Name);
    if(callF==nullptr){
        return nullptr;
...
llvm::Value *rmmc::FunctionDeclarationStatement::codeGen(CodeGenContext &context)
{
    ...
    FunctionPtr func = llvm::Function::Create(funcType, llvm::Function::ExternalLinkage, this->FunctionName->Name.c_str(), context.theModule.get());
    // entry block
    ...
    {
        perArg.setName((*it)->getName().Name);
        // ValuePtr argAlloc = (*it)->codeGen(context);
        // context.theBuilder.CreateStore(&perArg, &perArg, false);
        context.setSymbolTable((*it)->getName().Name, &perArg);
        context.setSymbolType((*it)->getName().Name, (*it)->getType());
        it++;
    }
...
llvm::Value *rmmc::SingleVariableDeclarationStatement::codeGen(CodeGenContext &context)
{
    ...
    context.setSymbolTable(this->VariableName->Name, alloca);
    context.setSymbolType(this->VariableName->Name, *this->VariableType);
...
{
    this->print();
    ArrayTypePtr type = llvm::ArrayType::get(getLLVMType(*this->ArrayType, context), ArraySize->Value);
    ValuePtr alloca = context.theBuilder.CreateAlloca(type);

    context.setSymbolTable(this->ArrayName->Name, alloca);
    context.setSymbolType(this->ArrayName->Name, *this->ArrayType);
